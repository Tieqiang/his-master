...
$(function () {
    ...
    var setButton = function(year,month){
        ...
        $.get("/api/acct-save-record/get?hospitalId="+parent.config.hospitalId+"&yearMonth="+yearM,function(data){
            ...
            if(data=="success"){
                ...
                $("#fetchItemBtn").linkbutton("enable") ;
            }
        })
        //查询结存记录
    }
    ...
    $("#fetchIncomeBtn").on('click', function () {
        $.messager.confirm('系统提示','重新提取HIS数据，将删除已经提取的本月份数据，是否继续？',function(data){
            if(data==1){
                ...
                $.post("/api/fetch-data-holiday/fetch-from-his?hospitalId=" + parent.config.hospitalId + "&yearMonth=" + yearMonth+"&fetchTypeId=节假日收入", function (data) {
                    $.messager.progress('close') ;
                    $("#fetchItemBtn").click();
                })
            }
    ...
    $("#fetchItemBtn").on('click', function () {
        ...
        options.url = "/api/fetch-data-holiday/fetch-by-page?hospitalId=" + parent.config.hospitalId + "&yearMonth=" + yearMonth;
    ...
    $("#devideIncomeBtn").on('click', function () {
        $.messager.confirm('系统提示','重新分割将重写以前分割数据，是否继续？',function(data){
            if(data==1){
                ...
                $.post("/api/fetch-data-holiday/devide-income?hospitalId="+parent.config.hospitalId+"&yearMonth="+yearMonth,function(data){
                    $.messager.progress('close')
                    $("#fetchItemBtn").click();
                })
            }
    ...
    $("#saveIncomeBtn").on('click', function () {
        ...
        //$.postJSON("/api/fetch-data-holiday/save-calc", rows, function (data) {
    ...
    $("#fetchType").combobox({
        ...
        valueField:'id',
        method:'GET',
        url:'/api/acct-param/list-by-type?hospitalId='+parent.config.hospitalId+"&fetchType=income_fetch_type_holiday",
        onLoadSuccess:function(){
            var data = $(this).combobox('getData') ;
            for(var i = 0 ;i<data.length;i++){
                if(data[i].paramName=='节假日收入'){
